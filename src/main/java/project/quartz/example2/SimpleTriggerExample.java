...
package project.quartz.example2;
...
public class SimpleTriggerExample {
  ...
  public void run() throws Exception {
    ...
    Date startTime = DateBuilder.nextGivenSecondDate(null, 7);
    Date ft;
    JobDetail job;
    SimpleTrigger trigger;
    // job1 will only fire once at date/time "ts"
    /* job = newJob(SimpleJob.class).withIdentity("job1", "group1").build();
     trigger = (SimpleTrigger) newTrigger().withIdentity("trigger1", "group1").startAt(startTime).build();
...
*/
    ...
    trigger = newTrigger().withIdentity("trigger8", "group1").startAt(startTime)
        .withSchedule(simpleSchedule().withIntervalInSeconds(2).withRepeatCount(20)).forJob(job).build();
